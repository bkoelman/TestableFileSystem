...
namespace TestableFileSystem.Fakes.Tests.Specs.FakeWatcher.NotifyFilter
{
    public sealed class NotifyDirectoryTimeLastWriteUtcSpecs : WatcherSpecs
    {
        ...
        [WatcherNotifyTestData(@"
            * Subfolder                                     @ LastWrite
        ")]
        private void When_changing_directory_last_write_time_in_UTC_it_must_raise_events(NotifyFilters filters,
        ...
        [WatcherNotifyTestData(@"
            * Subfolder                                     @ LastWrite
        ")]
        private void When_changing_directory_last_write_time_in_UTC_to_existing_value_it_must_raise_events(NotifyFilters filters,
            [NotNull] string expectedText)
        {
            // Arrange
            const string directoryToWatch = @"c:\some";
