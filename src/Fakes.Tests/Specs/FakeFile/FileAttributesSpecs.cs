...
namespace TestableFileSystem.Fakes.Tests.Specs.FakeFile
{
    public sealed class FileAttributesSpecs
    {
        ...
        private void When_getting_attributes_for_missing_remote_file_it_must_fail()
        {
            ...
            // Assert
            action.Should().Throw<FileNotFoundException>().WithMessage(@"Could not find file '\\server\share\missing.txt'.");
        }
        ...
        private void When_setting_attributes_for_existing_directory_to_Temporary_it_must_fail()
        {
            ...
            // Assert
            action.Should().Throw<ArgumentException>().WithMessage("Invalid File or Directory attributes value.*");
        }
