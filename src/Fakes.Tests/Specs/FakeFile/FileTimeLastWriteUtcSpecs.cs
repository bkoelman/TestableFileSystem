...
namespace TestableFileSystem.Fakes.Tests.Specs.FakeFile
{
    public sealed class FileTimeLastWriteUtcSpecs
    {
        ...
        private void When_getting_last_write_time_in_UTC_for_invalid_drive_it_must_fail()
        {
            ...
            // Act
            Action action = () => fileSystem.File.GetLastWriteTimeUtc("_:");
        ...
        private void When_setting_last_write_time_in_UTC_for_invalid_drive_it_must_fail()
        {
            ...
            // Act
            Action action = () => fileSystem.File.SetLastWriteTimeUtc("_:", DefaultTimeUtc);
        ...
        private void When_getting_last_write_time_in_UTC_using_absolute_path_without_drive_letter_it_must_succeed()
        {
            ...
            // Act
            DateTime time = fileSystem.File.GetLastWriteTimeUtc(@"\file.txt");
        ...
        [Fact, InvestigateRunOnFileSystem]
        private void When_getting_last_write_time_in_UTC_for_existing_relative_local_file_it_must_succeed()
        {
            // Arrange
            var clock = new SystemClock(() => DefaultTimeUtc);
        ...
        [Fact, InvestigateRunOnFileSystem]
        private void When_setting_last_write_time_in_UTC_for_existing_relative_local_file_it_must_succeed()
        {
            // Arrange
            IFileSystem fileSystem = new FakeFileSystemBuilder()
                .IncludingEmptyFile(@"C:\folder\some.txt")
        ...
        [Theory]
        [CanRunOnFileSystem]
        private void When_getting_last_write_time_in_UTC_for_file_below_missing_network_share_it_must_fail(bool useFakes)
        {
            using (var factory = new FileSystemBuilderFactory(useFakes))
            {
                // Arrange
        ...
        [Theory]
        [CanRunOnFileSystem]
        private void When_setting_last_write_time_in_UTC_for_file_below_missing_network_share_it_must_fail(bool useFakes)
        {
            using (var factory = new FileSystemBuilderFactory(useFakes))
            {
                // Arrange
