...
namespace TestableFileSystem.Fakes.Tests.Specs.FakeFile
{
    public sealed class FileTimeCreationSpecs
    {
        ...
        private void When_setting_creation_time_in_local_zone_for_existing_file_to_MaxValue_it_must_succeed()
        {
            ...
            // Act
            fileSystem.File.SetCreationTime(path, DateTime.MaxValue);

            // Assert
            fileSystem.File.GetCreationTime(path).Should().Be(DateTime.MaxValue);
        }
        ...
        [Fact]
        private void When_getting_creation_time_in_local_zone_for_exising_relative_local_file_it_must_succeed()
        {
            // Arrange
            var clock = new SystemClock { UtcNow = () => DefaultTimeUtc };
        ...
        [Fact]
        private void When_setting_creation_time_in_local_zone_for_exising_relative_local_file_it_must_succeed()
        {
            // Arrange
            IFileSystem fileSystem = new FakeFileSystemBuilder()
                .IncludingEmptyFile(@"C:\folder\some.txt")
