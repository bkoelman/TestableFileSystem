...
namespace TestableFileSystem.Fakes.Tests.Specs.FakeFile
{
    public sealed class FileTimeLastAccessUtcSpecs
    {
        ...
        private void When_setting_last_access_time_in_UTC_for_existing_file_to_MaxValue_it_must_succeed()
        {
            ...
            // Act
            fileSystem.File.SetLastAccessTimeUtc(path, DateTime.MaxValue);

            // Assert
            fileSystem.File.GetLastAccessTimeUtc(path).Should().Be(DateTime.MaxValue);
        }
        ...
        [Fact]
        private void When_getting_last_access_time_in_UTC_for_exising_relative_local_file_it_must_succeed()
        {
            // Arrange
            var clock = new SystemClock { UtcNow = () => DefaultTimeUtc };
        ...
        [Fact]
        private void When_setting_last_access_time_in_UTC_for_exising_relative_local_file_it_must_succeed()
        {
            // Arrange
            IFileSystem fileSystem = new FakeFileSystemBuilder()
                .IncludingEmptyFile(@"C:\folder\some.txt")
