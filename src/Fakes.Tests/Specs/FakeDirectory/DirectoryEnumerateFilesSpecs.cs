...
namespace TestableFileSystem.Fakes.Tests.Specs.FakeDirectory
{
    public sealed class DirectoryEnumerateFilesSpecs
    {
        ...
        private void When_enumerating_files_for_pattern_that_contains_parent_directory_with_asterisk_it_must_fail()
        {
            ...
            // Act
            Action action = () => fileSystem.Directory.EnumerateFiles(path, pattern);
        ...
        [Theory]
        [CanRunOnFileSystem]
        private void When_enumerating_files_for_missing_network_share_it_must_fail(bool useFakes)
        {
            using (var factory = new FileSystemBuilderFactory(useFakes))
            {
                // Arrange
        ...
        [Theory]
        [CanRunOnFileSystem]
        private void When_enumerating_files_below_missing_network_share_it_must_fail(bool useFakes)
        {
            using (var factory = new FileSystemBuilderFactory(useFakes))
            {
                // Arrange
